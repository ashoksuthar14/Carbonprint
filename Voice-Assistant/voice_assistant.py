...
Start by introducing yourself warmly and asking what activity they'd like to discuss."""
            ...
            try:
                ...
                if "429" in str(api_error) or "quota" in str(api_error).lower():
                    logger.error("API quota exceeded. Please check your Gemini API quota and billing details.")
                    return "I apologize, but I'm currently taking a short break due to high demand. Please try again in a few minutes! 🌱"
                raise
    ...
    def get_response(self, user_input):
        try:
            ...
            try:
                ...
                if "429" in str(api_error) or "quota" in str(api_error).lower():
                    logger.warning("API quota exceeded. Waiting before retry...")
                    return "I'm a bit overwhelmed right now. Could you please try again in a minute? 🌿"
                raise
        
        except Exception as e:
            logger.error(f"Error getting response: {str(e)}")
            return "I encountered an issue. Let's start fresh! What environmental topic would you like to discuss? 🌱"
